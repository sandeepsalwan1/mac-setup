...
class AutoreviewHardeningTests(unittest.TestCase):
    ...
    def test_tracked_source_names_and_env_templates_remain_reviewable(self) -> None:
        for rel in (
            ...
            ".env.example",
            "private/parser.py",
            ".agents/skills/example-secret-scanning-maintainer/SKILL.md",
    ...
    def test_claude_env_prefers_logged_in_subscription_over_api_key(self) -> None:
        with tempfile.TemporaryDirectory() as tempdir:
            repo = init_repo(Path(tempdir))
            ...
            with mock.patch.dict(
                self.helper["claude_engine_env"].__globals__,
                {
                    "safe_engine_env": lambda *_args, **_kwargs: {
                        "ANTHROPIC_API_KEY": "test-key"
                    },
                    "safe_temp_root": lambda _repo: Path(tempdir),
                    "run": lambda *_args, **_kwargs: status,
                },
            ):
                env = self.helper["claude_engine_env"](
                    repo,
                    "/usr/bin/claude",
                )
    ...
    def test_claude_env_keeps_api_key_without_subscription_login(self) -> None:
        with tempfile.TemporaryDirectory() as tempdir:
            repo = init_repo(Path(tempdir))
            status = subprocess.CompletedProcess(
                ["claude", "auth", "status"],
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            with mock.patch.dict(
                self.helper["run_claude"].__globals__,
                {
                    ...
                    "claude_engine_env": lambda *_args, **_kwargs: {
    ...
    def test_prepare_codex_keyring_access_skips_when_not_needed(self) -> None:
        with tempfile.TemporaryDirectory() as tempdir:
            root = Path(tempdir)
            runtime_home = root / "runtime-home"
            runtime_home.mkdir()
            with mock.patch.dict(self.helper, {"run": mock.Mock()}):
                self.helper["prepare_codex_keyring_access"](
                    root,
                    runtime_home,
                    root / "codex-home",
                    True,
                )
                self.helper["run"].assert_not_called()

    @unittest.skipUnless(sys.platform == "darwin", "requires macOS keychain")
    def test_prepare_codex_keyring_access_keeps_home_isolated(self) -> None:
        with tempfile.TemporaryDirectory() as tempdir:
            root = Path(tempdir)
            host_home = root / "host-home"
            runtime_home = root / "runtime-home"
            default_keychain = host_home / "Library" / "Keychains" / "login.keychain-db"
            default_keychain.parent.mkdir(parents=True)
            default_keychain.touch()
            runtime_home.mkdir()
            source_codex_home = host_home / ".codex"
            fake_run = mock.Mock(
                side_effect=[
            ...
            with (
                mock.patch.object(Path, "home", return_value=host_home),
                mock.patch.dict(
                    self.helper["prepare_codex_keyring_access"].__globals__,
                    {"run": fake_run},
                ),
            ):
                self.helper["prepare_codex_keyring_access"](
            ...
            for call in fake_run.call_args_list[1:]:
                self.assertEqual(call.kwargs["env"]["HOME"], str(runtime_home))
                self.assertEqual(call.kwargs["env"]["USERPROFILE"], str(runtime_home))
    ...
    def test_codex_command_falls_back_to_app_managed_current_binary(self) -> None:
        with tempfile.TemporaryDirectory() as tempdir:
            root = Path(tempdir)
            repo = init_repo(root)
            ...
            with mock.patch.dict(
                ...
                self.assertEqual(
                    self.helper["find_codex_command"]("codex", repo),
                    str(managed_bin.resolve()),
                )
                self.assertIsNone(
                    self.helper["find_codex_command"]("custom-codex", repo),
                )
