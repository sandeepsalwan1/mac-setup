---
name: autoreview
description: "Structured code review with Codex or Claude; optional Amp, Pi, or Kimi."
---

# Auto Review

Run the bundled structured review helper when the user asks for Autoreview, a second-model review, one of its named review engines, or repository rules explicitly require it.

Codex review is the default when no engine is set. It uses `gpt-5.6-sol` with `high` reasoning by default, then retries once with `gpt-5.6-terra` only when the account cannot access Sol. Claude review is optional and uses the future-updating `opus` alias by default. Amp review is optional and uses `openai/gpt-5.6-sol` with `high` reasoning by default. Pi and Kimi use the model configured by their respective CLIs unless `--model` overrides it.

Do not invoke Autoreview automatically unless the user or repository rules explicitly require it.
...
```bash
# Shared global skill for Codex and Claude:
export AUTOREVIEW="$HOME/.skills/autoreview/scripts/autoreview"
export AUTOREVIEW_HARNESS="$HOME/.skills/autoreview/scripts/test-review-harness"
```
...
| ------------------- | -------------------------------------------------- | ----------------------------------------------------- |
| **codex** (default) | `gpt-5.6-sol` -> `gpt-5.6-terra` on access failure | Autoreview default                                   |
| **claude**          | `opus`                                             | Latest available Opus                                 |
...
| **claude**          | `claude --model X`         | `opus`, `sonnet`, `haiku`, or a full model ID                               | `--effort Y`                              | `low`, `medium`, `high`, `xhigh`, `max`                    |
...
"$AUTOREVIEW" --engine claude --model opus --thinking max
"$AUTOREVIEW" --engine claude --model opus --fallback-model sonnet
